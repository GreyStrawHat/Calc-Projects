...
def setup():
    ...
    if os.path.isdir("./threadcalc_tests/"):
        shutil.rmtree("./threadcalc_tests/")
    os.mkdir("./threadcalc_tests/")

    if os.path.isdir("./threadcalc_tests/unsolved"):
        shutil.rmtree("./threadcalc_tests/unsolved")
    os.mkdir("./threadcalc_tests/unsolved")

    if os.path.isdir("./threadcalc_tests/solved"):
        shutil.rmtree("./threadcalc_tests/solved")
...
def main():
    ...
    infilelist = []
    outfilelist = []
    for dirpath, _, filenames in os.walk("./threadcalc_tests/unsolved"):
        for file in filenames:
            filepath = os.path.abspath(os.path.join(dirpath, file))
            if checkheader(filepath) == 0:
                infilelist.append(filepath)

    for dirpath, _, filenames in os.walk("./threadcalc_tests/solved"):
        for file in filenames:
            filepath = os.path.abspath(os.path.join(dirpath, file))
