...
def setup():
    ...
    if os.path.isdir("./netcalc_tests/"):
        shutil.rmtree("./netcalc_tests/")
    os.mkdir("./netcalc_tests/")

    if os.path.isdir("./netcalc_tests/unsolved"):
        shutil.rmtree("./netcalc_tests/unsolved")
    os.mkdir("./netcalc_tests/unsolved")

    if os.path.isdir("./netcalc_tests/solved"):
        shutil.rmtree("./netcalc_tests/solved")
    os.mkdir("./netcalc_tests/solved")

    os.system("python3 ./GeneratorsGraders/FileCalc/FileCalc_Generator.py -o ./netcalc_tests/unsolved")
    os.chdir("./5_NetCalc")
    os.system("./build.sh")
    os.system("./build/netcalc &")
...
def main():
    ...
    infilelist = []
    outfilelist = []
    for dirpath, _, filenames in os.walk("./netcalc_tests/unsolved"):
        for file in filenames:
            filepath = os.path.abspath(os.path.join(dirpath, file))
            if checkheader(filepath) == 0:
                infilelist.append(filepath)

    for dirpath, _, filenames in os.walk("./netcalc_tests/solved"):
        for file in filenames:
            filepath = os.path.abspath(os.path.join(dirpath, file))
    ...
    # returns negative failure on non passing netcalc_tests
    cleanup()
