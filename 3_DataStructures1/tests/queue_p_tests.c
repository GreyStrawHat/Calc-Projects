...
void test_queue_p_enqueue()
{
    ...
    //Should catch if enqueue_p is called on an invalid queue_p or with invalid data
...
void test_queue_p_destroy()
{
    ...
    //Should catch if delte is called on the queue_p that has already been deleted
    exit_code = queue_p_destroy(&queue_p);
    CU_ASSERT(0 != exit_code);
}
