...
void * compare_node(void * value_to_find, void * node)
{   
    void * retval = NULL;

    if (*(int *)((list_node_t *)node)->data == 
        *(int *)((list_node_t *)value_to_find)->data)
    {
        retval = node;
    }
...
void test_list_new()
{
    ...
    list = list_new(free_node, compare_node);
    CU_ASSERT_FATAL(NULL != list); //Function exited correctly
    CU_ASSERT(0 == list->size); //list size is correct
}
...
void test_list_push_back()
{
    ...
    CU_ASSERT(0 != list_push_head(invalid_list, &data[0]));

    while (i < 5)
    {
        exit_code = list_push_back(list, &data[i]);
        if (0 != exit_code)
        {
            break;
        }
        //New node was pushed and points to the correct data
        CU_ASSERT(data[i] == *(int *)(list->back->data));
        i++;
...
{
    CU_ASSERT(list->back->next == list->head && 
              list->head->prev == list->back);
}
...
void test_list_pop_back()
{
    ...
    //Should catch if pop is called on an invalid list
    node = list_pop_back(invalid_list);
    CU_ASSERT(NULL == node);

    //Pop all nodes
    while(i < 5)
    {
        node = list_pop_back(list);
...
void test_list_remove()
{
    ...
    //Should catch if remove is called on an invalid list
    exit_code = list_remove(invalid_list, 
...
void test_list_find_first_occurrence()
{
    ...
    //Should catch if function is called on an invalid list
    node = list_find_first_occurrence(invalid_list,
                                             (void *)&value_to_find);
    CU_ASSERT(NULL == node);

    //Should not be able to find the value removed in the previous test
    node = list_find_first_occurrence(list,
                                             (void *)&value_to_find);
    CU_ASSERT(NULL == node);
...
void test_list_find_all_occurrences()
{
    ...
    //Should catch if function is called on an invalid list
    result_list = list_find_all_occurrences(test_list, 
                                                  (void *)&value_to_find);
    CU_ASSERT(NULL != result_list);

    //create empty list
    test_list = list_new(free_node, compare_node);
...
void test_list_peek_back()
{
    ...
    //Should catch if pop is called on an invalid list
    node = list_peek_back(invalid_list);
    CU_ASSERT(NULL == node);

    //Function should not be able to peek on an empty list
    node = list_peek_back(list);
    CU_ASSERT(NULL == node);
...
void test_list_delete()
{
    ...
    //Should catch if delete is called on the list that has already been deleted
    exit_code = list_delete(&list);
    CU_ASSERT(0 != exit_code);
}
...
int main(void)
{
    CU_TestInfo suite1_tests[] = 
    {
        ...
        {"Testing list_push_back():", \
        test_list_push_back},

        {"Testing if the resulting list is circular:", \
        test_list_is_circular},

        {"Testing list_pop_head():", \
        test_list_pop_head},

        {"Testing list_push_head():", \
        test_list_push_head},
        ...
        {"Testing list_pop_back():", \
        test_list_pop_back},

        {"Testing list_peek_head():", \
        test_list_peek_head},
        ...
        {"Testing list_find_first_foreach_call():", \
        test_list_foreach_call},
